export class validation {
  ...
  static onlyNums(sString) {
    // returns true or false if a string contains only numbers
  ...
  static validEmail(sEmail) {
    ...
    for (let i = 0; i < sEmail.length; i++) {
      ...
      if (sEmail[i] == ".") {
        try {
          if ((sEmail[i + 1] == ".") | (sEmail[i + 1] == "@")) {
            // check letter after . is not another . or @
            flag_2 = false;
          }
        } catch (error) {
          // . was at end of email address so invalid email
          flag_2 = false;
        }
      }
    }
  ...
  static validName(sName) {
    // validate name - check every character in the name is an alphabetical letter
    if (sName == null) {
      return false;
    }
    if (sName.length == 0) {
    ...
    for (let i = 0; i < sName.length; i++) {
      if (
        ...
        //if character is not an alphabetical
  ...
  static validPhoneNum(sNum) {
    ...
    sNum = sNum.replace(" ", "");
    sNum = sNum.replace("+", "");
    if (!((sNum.length == 13) | (sNum.length == 10))) {
  ...
  static validDob(sDob) {
    ...
    if (current_date.getMonth() < user_date.getMonth()) {
      // birthday month not yet occcured
      age = age - 1;
    } else if (current_date.getMonth() == user_date.getMonth()) {
      //in birthday month
      if (current_date.getDate() < user_date.getDate()) {
        //birthday not yet occured
        age = age - 1;
      }
    }
    if (age >= 18 && age <= 75) {
      //user has to be 18-75 years old inclusive
      return true;
    } else {
      return false;
    }
  ...
  static validPassword(sString) {
    ...
    if (sString.length >= 6 && sString.length <= 15) {
      ...
      if (flag_lower && flag_number && flag_upper) {
        //password must satisfy all 3 conditions
        return true;
      } else {
        return false;
      }
