...
export default class QuestionBlockManager extends React.Component {
  ...
  render() {
    if (
      (this.props.list == "User is banned") |
      (this.props.list == "you are banned")
    ) {
      //user is banned so give relevant message
      return (
        <div>
          <label className="text-white">You are banned</label>
        </div>
      );
    }
    if (this.props.list.length > 0) {
          ...
          <div className="container1">
            ...
            {this.props.list.map((question) => {
              if (question.title != null && question.title != "") {
                return (
                  <Question_Block props={question} key={question.question_id} />
                );
              }
            })}
          </div>
      ...
      return (
        <div>
          <label className="text-white">No Questions to display</label>
        </div>
      );
    }
  }
